...
float pattern(vec2 p, out vec2 q, out vec2 r) {
  ...
  q = vec2(fbm(p + offset1), fbm(rotate(0.1 * time) * p + offset1));
...
vec3 getColor(vec2 p) {
  ...
  p.x += displace(p);
  if (uGlitchAmount != 1.0) p.x += displace(p) * (uGlitchAmount - 1.0);
...
void main() {
  ...
  if (uLightMode > 0.5) {
    ...
    col = mix(vec3(1.0), clamp(col * 0.42, 0.0, 0.76), coverage);
  }
...
export default function FaultyTerminal({
  ...
  const tintVector = useMemo(() => hexToRgb(tint), [tint]);
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uTint: { value: new Color(tintVector[0], tintVector[1], tintVector[2]) },
    ...
    const update = (timestamp: number) => {
      ...
      if (pageLoadAnimation && loadStartRef.current === 0) loadStartRef.current = timestamp;
      const shouldPause = pause || reducedMotion;
      if (!shouldPause) {
      ...
      if (pageLoadAnimation && loadStartRef.current > 0) {
        program.uniforms.uPageLoadProgress.value = Math.min((timestamp - loadStartRef.current) / 2000, 1);
      }
      if (mouseReact) {
  ...
  }, [brightness, chromaticAberration, curvature, digitSize, ditherValue, dpr, flickerAmount, glitchAmount, gridMul, handleMouseMove, lightMode, mouseReact, mouseStrength, noiseAmp, pageLoadAnimation, pause, scale, scanlineIntensity, timeScale, tintVector]);

  return <div ref={containerRef} className={`faulty-terminal-container ${className ?? ""}`} style={style} aria-hidden="true" {...rest} />;
}
